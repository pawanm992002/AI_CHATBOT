...
interface StudentRecord {
  ...
  summary: {
    ...
    pending_partial_due: string;
    pending_partial_fee_records: number;
...
const SchoolRecords = () => {
      ...
      <div className="space-y-6 text-slate-100 animate-fadeIn">
        ...
        <div className="border border-slate-800 bg-slate-900 p-5 sm:flex sm:items-center sm:justify-between sm:gap-6">
          ...
          <div className="mt-4 border-l-2 border-rose-400 pl-4 sm:mt-0">
            ...
            <p className="mt-1 font-mono text-2xl font-bold text-rose-300">{money(record.summary.outstanding_balance)}</p>
            <p className="mt-1 text-xs text-slate-500">Net fees after all recorded payments</p>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {([
            ...
            { label: 'Payments received', value: money(record.summary.total_paid), icon: WalletCards, color: 'text-emerald-300' },
            { label: 'Pending / partial records', value: String(record.summary.pending_partial_fee_records), icon: GraduationCap, color: 'text-amber-300' },
        ...
        <div className="grid gap-3 border-l-2 border-violet-400 bg-violet-950/20 px-4 py-3 text-sm text-slate-300 sm:grid-cols-2">
          <p><span className="font-semibold text-white">Outstanding balance:</span> Net assigned fees minus every recorded payment.</p>
          <p><span className="font-semibold text-white">Pending / partial amount:</span> {money(record.summary.pending_partial_due)} across {record.summary.pending_partial_fee_records} fee record{record.summary.pending_partial_fee_records === 1 ? '' : 's'}.</p>
          <p className="sm:col-span-2 text-xs text-slate-400">{record.summary.calculation_basis}</p>
        </div>
        ...
        <Section title="Applied fees">
          {record.fees.length ? <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-slate-800 text-xs uppercase tracking-wider text-slate-500"><tr><th className="pb-3 font-semibold">Fee head</th><th className="pb-3 font-semibold">Assigned</th><th className="pb-3 font-semibold">Concession</th><th className="pb-3 font-semibold">Net</th><th className="pb-3 font-semibold">Paid</th><th className="pb-3 font-semibold">Outstanding</th><th className="pb-3 font-semibold">Due date</th><th className="pb-3 font-semibold">Status</th></tr></thead><tbody>{record.fees.map(fee => <tr key={String(fee.applied_fee_id)} className="border-b border-slate-800/70 last:border-0"><td className="py-3 text-slate-200">{String(fee.fee_head)}</td><td className="py-3 font-mono text-slate-300">{money(fee.amount)}</td><td className="py-3 font-mono text-slate-300">{money(fee.concession)}</td><td className="py-3 font-mono text-slate-300">{money(Number(fee.amount || 0) - Number(fee.concession || 0))}</td><td className="py-3 font-mono text-emerald-300">{money(fee.paid_amount)}</td><td className="py-3 font-mono font-semibold text-white">{money(fee.outstanding_amount)}</td><td className="py-3 text-slate-400">{String(fee.due_date || '-')}</td><td className="py-3"><span className={`border px-2 py-1 text-xs font-semibold ${statusClass(fee.status)}`}>{String(fee.status)}</span></td></tr>)}</tbody></table></div> : <p className="text-sm text-slate-500">No applied fee records.</p>}
        </Section>
  ...
  return (
    <div className="space-y-6 text-slate-100 animate-fadeIn">
      <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center bg-violet-600 text-white"><UserRound size={20} /></div><div><h2 className="text-2xl font-bold text-white">School Records</h2><p className="mt-1 text-sm text-slate-400">Search and view student, fee, payment, transport, and hostel records for your school.</p></div></div>
