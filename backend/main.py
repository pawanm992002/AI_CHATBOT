...
from controllers import tenants, crawl, chat, sources, faqs, text_docs, leads, admin, knowledge_improvement, providers, admin_analytics, visitor_profiles, conversations, school_dashboard
...
app.include_router(school_dashboard.router, prefix="/api")
...
async def ensure_lookup_indexes():
    try:
        ...
        # School ERP indexes
        await db.schools.create_index([("tenant_id", 1)], unique=True)
        await db.school_classes.create_index([("tenant_id", 1), ("class_id", 1)])
        await db.school_classes.create_index([("tenant_id", 1), ("school_id", 1)])
        await db.school_sections.create_index([("tenant_id", 1), ("section_id", 1)])
        await db.school_sections.create_index([("tenant_id", 1), ("class_id", 1)])
        await db.school_students.create_index([("tenant_id", 1), ("student_id", 1)])
        await db.school_students.create_index([("tenant_id", 1), ("admission_no", 1)])
        await db.school_students.create_index([("tenant_id", 1), ("class_id", 1)])
        await db.school_students.create_index([("tenant_id", 1), ("student_name", 1)])
        await db.school_routes.create_index([("tenant_id", 1), ("route_id", 1)])
        await db.school_stops.create_index([("tenant_id", 1), ("stop_id", 1)])
        await db.school_stops.create_index([("tenant_id", 1), ("route_id", 1)])
        await db.school_transport_assign.create_index([("tenant_id", 1), ("transport_id", 1)])
        await db.school_transport_assign.create_index([("tenant_id", 1), ("student_id", 1)])
        await db.school_hostel_assign.create_index([("tenant_id", 1), ("hostel_id", 1)])
        await db.school_hostel_assign.create_index([("tenant_id", 1), ("student_id", 1)])
        await db.school_applied_fees.create_index([("tenant_id", 1), ("applied_fee_id", 1)])
        await db.school_applied_fees.create_index([("tenant_id", 1), ("student_id", 1)])
        await db.school_applied_fees.create_index([("tenant_id", 1), ("status", 1)])
        await db.school_payments.create_index([("tenant_id", 1), ("payment_id", 1)])
        await db.school_payments.create_index([("tenant_id", 1), ("student_id", 1)])
        await db.school_payments.create_index([("tenant_id", 1), ("applied_fee_id", 1)])
        await db.school_data_query_log.create_index([("tenant_id", 1), ("timestamp", -1)])
    except Exception as e:
